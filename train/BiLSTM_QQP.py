...
qqp = load_from_disk("./QQP")

len(qqp['train'])

sentences = qqp['train']['question1']+qqp['train']['question2']
combined_logits = qqp['train']['combined_logits']
...
logits = np.array(qqp['train']['combined_logits'])
...
def preprocess_batch(batch_data):
  # print(batch_data)
  question1s, question2s, combined_logits = batch_data['question1'], batch_data['question2'], batch_data['combined_logits']
  print('converting to tf tensor')
  question1s_data = embedding_layer(vectorizer(question1s)).numpy()
  question2s_data = embedding_layer(vectorizer(question2s)).numpy()
  
  padded_sequences = np.concatenate((question1s_data, question2s_data, question1s_data*question2s_data, tf.math.abs(question1s_data - question2s_data)), axis=1)
  # print(padded_sequences)
...
a, b = preprocess_batch(qqp['train'])
...
def get_test_data(data):
  sentences1, sentences2, labels = data['question1'], data['question2'], data['label']
...
test_padded_sequences, true_lables = get_test_data(qqp['validation'])
...
for i, f in enumerate(flag):
  if f:
    identified_values.append((qqp['validation'][i]['question1'], qqp['validation'][i]['question2'], qqp['validation'][i]['label']))

for i, f in enumerate(flag):
  if not f:
    missed_values.append((qqp['validation'][i]['question1'], qqp['validation'][i]['question2'], qqp['validation'][i]['label']))
